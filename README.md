...
A structured log of LeetCode practice problems, organized by algorithmic topic and maintained as a public portfolio. This repository demonstrates consistency, breadth of coverage, and systematic approach to technical interview preparation.
...
This section is automatically maintained by a GitHub Actions workflow that runs on every new LeetSync commit. The tracker reads the raw solution files, computes statistics, and rewrites the Progress section in place — without modifying any solution code.

To add a new problem to the tracker, update `config/topics.json` and `config/titles.json` with the problem number, topic, and canonical title.
...
Raw solution files are generated by [LeetSync](https://github.com/isamert/leetcode-sync) and are never renamed or modified by this repository's tooling. The only organizational layer added is the topic folder.
...
**Per-problem folder structure:**

```
python/{topic}/{N}-{problem-name}/
    {problem-name}.py   # solution file (LeetSync-generated name, untouched)
    README.md           # problem statement (LeetSync-generated, untouched)
```
...
|---------|------------|---------|
| Problem folder | LeetSync format — number prefix, kebab-case | `1-two-sum/` |
| Solution file | LeetSync format — kebab-case | `two-sum.py` |
| Topic folder | `lowercase_snake_case` | `arrays_and_hashing/` |
| Language folder | language name | `python/` |

New problems synced by LeetSync are placed directly into the appropriate topic folder with no renaming required.
